...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    const loadTask = async () => {
      ...
      setGithubBranchUrl(task.github_branch_url || "");
      setAssignedTo(task.assigned_to || "");
      
      // Разбиваем теги из строки
      if (task.tag) {
        setTags(task.tag.split(",").map((tag: string) => tag.trim() ).filter((tag: any) => tag ));
      }
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-bold text-white mb-2">Редактировать задачу</h1>
        <p className="text-gray-400">Измените параметры задачи</p>
      </motion.div>
